...
//    private NavigationMapRoute navigationMapRoute;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pay = new Intent(this, com.example.TollPayActivity.MainActivity.class);
...
//                    // end animate camera
                    ...
                    String search_1 = getString(R.string.search_box_instructions);
                    String search_2 = getString(R.string.search_box_instructions_alt);
                    String route_complete = getString(R.string.route_complete_text);

                    if (tempMarkerCount == 0) {
                        sourcePoint = Point.fromLngLat(((Point) selectedCarmenFeature.geometry()).longitude(),
                                ((Point) selectedCarmenFeature.geometry()).latitude());
                        sourceMarker = mapboxMap.addMarker(new MarkerOptions()
                                .position(new LatLng(((Point) selectedCarmenFeature.geometry()).latitude(),
                                ((Point) selectedCarmenFeature.geometry()).longitude())));

                        tempMarkerCount += 1;
                        // update search_text
                        search_text.setText(search_2);
                        ...
                        tempMarkerCount = 0;
                        // update search_text
                        search_text.setText(route_complete);
                        findViewById(R.id.fab_location_source).setVisibility(View.INVISIBLE);
                        findViewById(R.id.search_text).setTextAlignment(View.TEXT_ALIGNMENT_VIEW_START);
                        (search_text)
                                .setBackgroundColor((TRANSPARENT));
                        (search_text).setTextColor(BLUE);
                        (search_text).setTypeface(Typeface.DEFAULT_BOLD);
    ...
    public  void onCheckboxClicked(View view){
        ...
        switch (view.getId()) {
            ...
            case R.id.toll1:
                if (checked)
                {
                    ((TollApplication) getApplicationContext()).tollArray[0] = 1;
                    count += 1;
                }
                break;
            // toll 2 clicked
            case R.id.toll2:
                if (checked)
                {
                    ((TollApplication) getApplicationContext()).tollArray[1] = 1;
                    count += 1;
                }
                break;
            // toll 3 clicked
            case R.id.toll3:
                if (checked)
                {
                    ((TollApplication) getApplicationContext()).tollArray[2] = 1;
                    count += 1;
                }
                break;
            // toll 4 clicked
            case R.id.toll4:
                if (checked)
                {
                    ((TollApplication) getApplicationContext()).tollArray[3] = 1;
                    count += 1;
                }
                break;
        }
    ...
    public void payButton(){
        findViewById(R.id.pay_button).setOnClickListener(new View.OnClickListener() {
            TollApplication tollApp = ((TollApplication) getApplicationContext());
            @Override
            public void onClick(View view) {
                if (ready_to_pay){
                    ...
                    startActivity(new Intent(MainActivity.this, com.example.TollPayActivity.MainActivity.class));
                }
                else{
                    Context context = getApplicationContext();
                    CharSequence noTolls = "No Tolls Selected!";
                    int duration = Toast.LENGTH_SHORT;

                    Toast toast = Toast.makeText(context, noTolls, duration);
